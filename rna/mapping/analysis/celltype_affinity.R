...
sample_metadata <- fread("/Users/ricard/data/gastrulation/rna/results/mapping/mapping.txt.gz") %>%
  .[,c("id_rna","celltype.mapped")] %>%
  setnames(c("id_rna","lineage10x")) %>%
  .[lineage10x%in%opts$celltypes]

# Load metadata
# sample_metadata <- sample_metadata %>% 
#   .[pass_rnaQC==T & lineage10x%in%opts$celltypes] %>%
#   .[!is.na(lineage10x)] %>%
...
sce$celltype.mapped <- sample_metadata$lineage10x
