...
io$in.sample_metadata <- "/Users/ricard/data/gastrulation/sample_metadata.txt"
io$in.raw_counts <- "/Users/ricard/data/gastrulation/rna/counts/counts.txt.gz"
io$out.file <- "/Users/ricard/data/gastrulation/rna/parsed/SingleCellExperiment.rds"
io$out.sample_metadata <- "/Users/ricard/data/gastrulation/sample_metadata.txt"
```


```{r load_data, echo=FALSE}
# counts <- read.table(sprintf("%s",io$in.raw_counts), header=T) 
counts <- fread(cmd=sprintf("zcat < %s",io$in.raw_counts)) %>% matrix.please
```


```{r load_sample_metadata, echo=FALSE}
sample_metadata = fread(io$in.sample_metadata, header=T, stringsAsFactors = F) %>% 
  setkey(id_rna)
```
...
# number.drop <- sce$number %in% c("triple","multiple")
```

```{r}
# p12 <- cowplot::plot_grid(p1, p2, ncol=1, nrow=2)
# pdf(file="/Users/ricard/gastrulation/qc_expr.pdf", width = 8, height = 5)
# print(p12)
# dev.off()

# p34 <- cowplot::plot_grid(p3, p4, ncol=2, nrow=1)
# pdf(file="/Users/ricard/data/gastrulation_NMT/qc_expr2.pdf", width = 15, height = 7)
# print(p34)
# dev.off()
```
...
# Exclude particular cells
# drop.samples["lane6504_AGGCAGAA_AAGGAGTA_Plate11_E03_L001_R1_val_1_GRCm38_ERCC_hisat2"] == F
