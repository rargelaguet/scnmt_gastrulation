...
io$outdir <- file.path(io$basedir,"results/rna/differential/marker_genes/germ_layers"); dir.create(io$outdir, showWarnings = F)
...
opts$celltypes <- c("E7.5_Ectoderm", "E7.5_Endoderm", "E7.5_Mesoderm")
...
sample_metadata <- fread(io$metadata) %>% 
  ...
  .[stage_lineage3%in%opts$celltypes]
...
foo <- diff.dt[,.(score=sum(direction=="up")), by=c("celltypeA","gene")] %>% setnames("celltypeA","celltype")
bar <- diff.dt[,.(score=sum(direction=="down")), by=c("celltypeB","gene")] %>% setnames("celltypeB","celltype")
...
# Save
fwrite(markers_genes.dt[,score:=NULL], file.path(io$outdir,"marker_genes_up.txt.gz"))

to.save <- diff.dt[gene%in%unique(markers_genes.dt$gene)] %>% .[,direction:=NULL]
fwrite(to.save, file.path(io$outdir,"diff_results.txt.gz"))
