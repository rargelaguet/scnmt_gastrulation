---
title: "Gastrulation: local correlation between chromatin accessibility and RNA expression in ESC"
output: 
...
io$outdir <- "/Users/ricard/gastrulation/accrna/local_coupling/comparison/ESC/out"
...
io$rna.infile <- paste(io$basedir,"rna/parsed/sce.rds",sep="/")
...
opts$cells <- fread(io$in.sample_metadata) %>% 
  .[pass_accQC==T & pass_rnaQC==T & culture%in%c("Serum","2i"),sample] 
```
...
rna_dt <- rna_dt %>% merge(sample_metadata[,c("sample","culture")], by="sample")
```

<!-- Merge accessibility and RNA expression data -->
```{r}
accrna <- merge(acc_dt, rna_dt, by=c("sample","ens_id","culture"))
rm(acc_dt,rna_dt)
```
...
bar <- foverlaps(x=foo %>% setkey(rel_start,rel_end),
  ...
  .[,.(rate=mean(rate), N=.N), by=c("sample","ens_id","gene","window_center","expr","culture")]
rm(tmp,foo,accrna)
```
...
cor <- bar %>% 
  .[,.(r=cor(x=rate, y=expr, method="pearson", use="pairwise.complete.obs")),
    by=c("sample","window_center","culture")]
```

<!-- Plot -->
```{r}
p <- ggplot(cor,aes(x=window_center, y=r, fill=culture, color=culture)) +
...
pdf(file=paste0(io$outdir,"/accrna_localcoupling_ESC.pdf"), width=5, height=3.5)
print(p)
dev.off()
```
