...
boxplot_theme <- function() {
  p <- theme(
    ...
    axis.title.x = element_text(colour="black", size=15, vjust=1.5, margin=margin(5,0,0,0)),
...
gg_volcano_plot <- function(cor_samples, title = "", label=0){
  # p <- ggplot(cor_samples, aes(x=r, y=-log10(p))) +
  p <- ggplot(cor_samples, aes(x=r, y=log_padj_fdr)) +
    labs(title=title, x="Pearson correlation", y=expression(paste("-log"[10],"(",plain(p),")"))) +
    ...
    annotate("text", x=0, y=7.95, size=7, label=sprintf("(%d)", all)) +
    annotate("text", x=-0.5, y=7.95, size=7, label=sprintf("%d (-)",length(negative_hits))) +
    annotate("text", x=0.5, y=7.95, size=7, label=sprintf("%d (+)",length(positive_hits))) +
    scatter_theme()
