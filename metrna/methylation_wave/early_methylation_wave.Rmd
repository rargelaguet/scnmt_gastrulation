---
title: "Gastrulation: joint characterisation of DNA methylation and RNA expression changes in the early methylation wave (E4.5 to E5.5)"
output:
...
diffmet <- fread(sprintf("zcat < %s/E4.5EPI_vs_E5.5EPI.tsv.gz",io$diffmet.dir)) %>% .[anno%in%names(opts$annos)]
```
...
```{r}
diffmet_filt <- diffmet %>% copy %>% .[,met.sig:=(p.value<0.01 & abs(propdiff)>0.25)] %>% .[,c("id","anno","met.sig","propdiff")] %>%
...
p <- ggplot(tmp, aes(x=rate, y=expr)) +
  ...
  geom_text(data = r, aes(x=50, y=5, label = r)) +
  geom_text(data = r, aes(x=50, y=6, label = pval)) +
...
p
# pdf(paste0(io$outdir,"/dnmt3b_cor2.pdf"), width=10, height=3.5, useDingbats = F)
# print(p)
# dev.off()
```
