...
if (grepl("ricard",Sys.info()['nodename'])) {
  ...
  io$outdir <- "/Users/ricard/gastrulation/metrna/local_coupling/out"
...
tss <- tss[,type:=ifelse(ens_id%in%non_cgi_promoters,'non_cgi','cgi')]
...
sample_metadata <- fread(io$in.sample_metadata, stringsAsFactors=TRUE) %>% 
  ...
  .[,c("sample","id_met","id_rna","stage","lineage","stage_lineage")]
...
met <- loadMultipleMetData(io$met.indir, opts$met.cells, tss) %>% .[,type:=NULL]
```
...
```{r}
met <- met %>% merge(sample_metadata[,c("id_met","sample","stage","stage_lineage")], by="id_met")
rna_dt <- rna_dt %>% merge(sample_metadata[,c("id_rna","sample","stage","stage_lineage")], by="id_rna")
```

<!-- Merge DNA methylation and RNA expression data -->
```{r}
metrna <- merge(met, rna_dt, by=c("sample","ens_id","stage","stage_lineage")) %>% .[,c("id_met","id_rna"):=NULL]
rm(met,rna_dt)
```
...
```{r}
cor <- bar %>% .[,.(r=cor(x=rate, y=expr, method="pearson", use="pairwise.complete.obs")),
                 by=c("sample","window_center","stage")]
```
...
p <- ggplot(cor,aes(x=window_center, y=r)) +
  ...
  # geom_line(aes(group=interaction(stage_lineage,type), color=stage_lineage), alpha=1.0, size=1.0) +
