...
met_dt <- lapply(names(opts$annos), function(n) {
  data <- fread(cmd=sprintf("zcat < %s/%s.tsv.gz",io$met_dir,n)) %>%
...
if (opts$permutation) {
  ...
  for (k in 1:opts$n_perms){
    ...
    # metrna_dt_perm <- metrna_dt_perm[,c("expr","rate"):=list(sample(expr),sample(rate)), by=c("id","gene","anno")]
...
<!-- Scatterplots of single examples -->
```{r}
# anno_foo="genebody"
# id_foo="ENSMUSG00000019863"
# 
# p <- ggplot(metrna_dt[anno==anno_foo & id==id_foo], aes(x=rate,y=expr)) +
