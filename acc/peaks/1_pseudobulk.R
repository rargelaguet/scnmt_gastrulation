...
io$out_dir <- paste0(io$data_dir, "/acc/raw/pseudobulk/")


## options ##
opts <- list()

opts$stages <- "E4.5"#"all"#c("E4.5", "E5.5", "E6.5", "E7.5")
opts$lineages <- "all"#"PE"#"Ectoderm" #"all" #"Endoderm" #"all"
opts$parallel <- TRUE
opts$chunk <- 20 # for parallel processing split input files into chunks of this many (smaller chunk size requires more memory)
opts$format <- "tsv" # cov = <chr><start><end><rate><met_reads><nonmet_reads> defaults to <chr><pos><met_reads><total_reads>
opts$gzip <- TRUE
