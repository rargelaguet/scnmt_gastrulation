---
title: "Gastrulation: general accessibility statistics of single cells"
output:
...
## Define I/O 
io <- list()
# io$in.metadata <- "/Users/ricard/data/gastrulation/sample_metadata_scNMT_pseudobulk.txt"
# io$in.data <- "/Users/ricard/data/gastrulation/acc/raw/pseudobulk"
...
io$outdir <- "/Users/ricard/data/gastrulation/acc/stats/samples"
...
# Define which cells to use
tmp <- fread(io$in.metadata) %>% .[!is.na(id_acc)] 
...
```{r}
metadata <- fread(io$in.metadata,stringsAsFactors=F) %>% 
...
<!-- Mean accessibility rate per stage -->
```{r}
...
p <- ggboxplot(stats_filt, x = "embryonic", y = "mean_regressed", fill="#00BFC4", coef=1, outlier.shape=NA) +
