...
opts$anno_select = c("ATF2", "E4F1", "CREB1", "ZFHX3", "ARID3H")
opts$filter_by_tf_exp <- FALSE # include only motifs from expressed TFs?
...
if (opts$filter_anno) files <- map(opts$anno_select, ~files[grep(., toupper(files))]) %>% unlist
