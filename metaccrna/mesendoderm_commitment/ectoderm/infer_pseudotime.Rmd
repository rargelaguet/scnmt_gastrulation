...
```{r}
# tmp <- fread(paste0(io$outdir,"/destiny_ectoderm.tsv"))
```
...
p <- ggplot(tmp, aes(rank_x,rank_z)) +
  geom_point(aes(color=stage_lineage), alpha=1.0, size=1.0) +
  geom_rug(aes(color=stage_lineage), sides="b") + 
...
# pdf(paste0(io$outdir,"/destiny_ectoderm_v2.pdf"), useDingbats = F, width=4.75, height=4.25)
# print(p)
# dev.off()
```
...
# write.table(tmp, paste0(io$outdir,"/destiny_ectoderm.tsv"), quote=F, sep="\t", row.names = F, col.names = T)
```
