...
```{r}
io$pdfdir2 <- paste0(io$pdfdir,"/characterisation_factors/ExE_endoderm_factor")
opts$width <- 4.5
opts$height <- 4
...
```{r}
plot_variance_explained(model, x="group", y="view", factors=factor, legend = T, max_r2 = 0.25, use_cache = T)
```
...
Top gene with nlegative loadings
```{r}
# bottom.gene <- names(head(sort(get_weights(model, views="RNA expression", factor=factor)[[1]][,1]), n=1))
# 
# p <- plot_factor(model,
#   factor = factor,
#   color_by = bottom.gene,
#   legend = FALSE,
#   dot_size = dot_size,
...
```{r}
# top.gene <- names(tail(sort(get_weights(model, views="RNA expression", factor=factor)[[1]][,1]), n=1))
# 
# p <- plot_factor(model,
#   factor = factor,
#   color_by = top.gene,
...
#   )
#   
# pdf(paste0(io$pdfdir2,"/Factor_rna_top.pdf"), width=opts$width/1.5, height=opts$height/2, useDingbats = F)
# print(p)
# dev.off()
```
