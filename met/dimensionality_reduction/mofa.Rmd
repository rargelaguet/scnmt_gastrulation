---
title: "Gastrulation: dimensionality reduction on the DNA methylation data"
output:
...
#          setnames(c("id_met","id","anno","N","rate")) %>% .[N>=opts$min.CpGs] %>% .[,N:=NULL]) %>% rbindlist
#   ) %>% rbindlist
# } else {
#   met_dt <- lapply(names(opts$annos), function(n) 
...
# pdf(sprintf("%s/pdf/mofa_met_%s_%s.pdf",io$outdir,paste(names(opts$annos), collapse="_"), paste(opts$stage_lineage10x, collapse="_")), useDingbats = F, onefile = F, width=6, height=6)
# print(p)
# dev.off()
```

<!-- Correlate Factor 1 with global methylation differences -->
```{r}
# library(ggpubr) 
# 
# foo <- fread(io$met.stats) %>% .[,c("id_met","mean")] %>% 
#   merge(sample_metadata_filt, by="id_met") %>%
#   setnames("id_met","sample") %>% .[,mean:=100*mean] %>%
...
#   stat_cor(method = "pearson", label.x = -2.5, label.y = 90) +
#   labs(x="Factor 1", y="Mean methylation rate") +
#   scale_colour_manual(values=opts$colors) +
#   theme(
#     axis.title = element_text(size=rel(1.2)),
#     legend.position = "none"
#   )
#  
# pdf(sprintf("%s/pdf/mofa_met_CORRELATION.pdf",io$outdir), useDingbats = F, width=6, height=6)
# print(p)
# dev.off()
```
...
# pdf(sprintf("%s/pdf/mofa+umap_met_%s_%s.pdf",io$outdir,paste(names(opts$annos), collapse="_"), paste(opts$stage_lineage10x, collapse="_")), useDingbats = F, onefile = F, width=6, height=6)
print(p)
# dev.off()
}
```
