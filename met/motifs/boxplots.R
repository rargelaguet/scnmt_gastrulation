...
# I/O
io$outdir <- paste0(io$basedir,"/met/results/motifs/test")
...
for (i in unique(to.plot$motif)) {
  ...
  pdf(sprintf("%s/%s_boxplots_met_lineage2.pdf",io$outdir,i), width=6, height=3)
  print(p)
  dev.off()
}
...
for (i in unique(to.plot$motif)) {
  ...
  pdf(sprintf("%s/%s_boxplots_met_lineage1.pdf",io$outdir,i), width=8, height=3)
  print(p)
  dev.off()
}
