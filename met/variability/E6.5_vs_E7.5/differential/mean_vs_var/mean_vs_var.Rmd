...
opts$anno <- c(
  "H3K27ac_distal_E7.5_union_intersect12_500" = "Enhancers",
  "H3K27ac_distal_E7.5_union_intersect12" = "Enhancers (500bp)",
  # "H3K4me3_E7.5_union",
...
# Define groups
opts$groups <- c("E6.5_Epiblast", "E7.5_Endoderm")
...
diff.mu.dt <- names(opts$anno) %>% 
  ...
  map(~ fread(sprintf("%s/%s_%s_%s_mu.txt.gz", io$diff.dir,.,opts$groups[1],opts$groups[2]))) %>%
...
diff.resdisp.dt <- names(opts$anno) %>%
  ...
  map(~ fread(sprintf("%s/%s_%s_%s_epsilon.txt.gz", io$diff.dir,.,opts$groups[1],opts$groups[2]))) %>%
...
diff.disp.dt <- names(opts$anno) %>% 
  ...
  map(~ fread(sprintf("%s/%s_%s_%s_gamma.txt.gz", io$diff.dir,.,opts$groups[1],opts$groups[2]))) %>%
