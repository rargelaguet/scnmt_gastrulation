...
Divide genomic elements into the four quadrants
```{r}
diff.dt %>%
  .[mean_diff>0.1 & disp_diff>0,quadrant:="2"] %>%
  .[mean_diff>0.1 & disp_diff<0,quadrant:="4"] %>%
  .[mean_diff<(-0.1) & disp_diff<0,quadrant:="3"] %>%
  .[mean_diff<(-0.1) & disp_diff>0,quadrant:="1"]

table(diff.dt$quadrant)
```
...
```{r}
to.plot.met <- met_dt %>%
  merge(diff.dt[,c("id","anno","quadrant")], by=c("id","anno")) %>%
  .[,.(rate=100*(sum(Nmet)/sum(Ntotal)), Nmet=sum(Nmet), N=sum(Ntotal)),by=c("sample","stage_lineage","anno","quadrant")]

to.plot.met <- to.plot.met[!is.na(quadrant)]
```
...
p <- ggplot(to.plot.met, aes(x=stage_lineage, y=rate)) +
  ...
  facet_wrap(~anno+quadrant, nrow=2, scales="fixed") +
