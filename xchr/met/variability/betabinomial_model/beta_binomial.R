...
if (opts$filter.var==TRUE) {
  keep_var_sites <- data %>% split(.$anno) %>% map(~ .[,.(var=var(rate)), by="id"] %>% .[var>0] %>% .$id)
  data <- data %>% split(.$anno) %>% map2(.,names(.), function(x,y) x[id %in% keep_var_sites[[y]]]) %>% rbindlist
}
