...
for (i in unique(to.plot$anno)) {
  ...
  pdf(sprintf("%s/Tal1_acc_vs_expr_per_cell_%s.pdf",io$outdir,i), width=7, height=4)
  print(p)
  dev.off()
}
```
...
## Boxplots stratifying by Tal1 expression
...
for (i in unique(to.plot$anno)) {
  ...
  p <- ggplot(to.plot[anno==i], aes(x=Tal1, y=rate, fill=Tal1)) +
  ...
  pdf(sprintf("%s/Tal1_boxplots_met_stratified_%s.pdf",io$outdir,i), width=6, height=4)
  print(p)
  dev.off()
}
```
...
for (i in unique(to.plot$anno)) {
  ...
  pdf(sprintf("%s/Tal1_boxplots_acc_stratified_%s.pdf",io$outdir,i), width=6, height=4)
  print(p)
  dev.off()
}
```
