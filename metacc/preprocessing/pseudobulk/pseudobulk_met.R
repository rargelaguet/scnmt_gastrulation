...
if (grepl("ricard",Sys.info()['nodename'])) {
  ...
  io$out.dir <- "/Users/ricard/data/gastrulation/met/raw/pseudobulk"
} else {
  io$in.data <- "/hps/nobackup/stegle/users/ricard/gastrulation/met/raw"
  io$in.metadata <- "/hps/nobackup/stegle/users/ricard/gastrulation/sample_metadata_scNMT_lenient.txt"
  io$out.dir <- "/hps/nobackup/stegle/users/ricard/gastrulation/met/raw/pseudobulk"
}
...
opts$cells <- fread(io$in.metadata) %>% 
  ...
  .[pass_metQC==T & group%in%opts$groups, id_met]
...
sample_metadata <- fread(io$in.metadata,stringsAsFactors=T)[,c("stage","lineage","id_met")] %>% 
