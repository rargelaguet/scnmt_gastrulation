...
Make a separate plot for every genomic context
```{r}
# for (i in unique(foo.met$anno)) {
#   p <- ggplot(foo.met[anno==i], aes(x=stage_lineage, y=rate)) +
#     geom_boxplot(aes(fill=stage_lineage), outlier.shape=NA) +
#     # scale_fill_manual(values=opts$colors) +
#     facet_wrap(~stage, nrow=2, scales="free_x") +
#     labs(x="", y="Methylation rate", title=i) +
...
#   print(p)
#   
#   # pdf(sprintf("%s/boxplots_met_%s.pdf",io$outdir,i), useDingbats = F, onefile = F, width=8, height=8)
#   print(p)
#   # dev.off()
# }
# 
# for (i in unique(foo.acc$anno)) {
#   p <- ggplot(foo.acc[anno==i], aes(x=stage_lineage, y=rate)) +
#     geom_boxplot(aes(fill=stage_lineage), outlier.shape=NA) +
#     # scale_fill_manual(values=opts$colors) +
#     facet_wrap(~stage, nrow=2, scales="free_x") +
#     labs(x="", y="Accessibility rate", title=i) +
#     coord_cartesian(ylim=c(20,60)) +
#     guides(color=F, fill=F) +
#     theme_bw() + theme_pb() +
#     theme(strip.background = element_rect(fill="#00BFC4"))
# 
#   # pdf(sprintf("%s/boxplots_acc_%s.pdf",io$outdir,i), useDingbats = F, onefile = F, width=8, height=8)
#   print(p)
#   # dev.off()
# }
```
