---
title: "Gastrulation: local coupling between DNA methylation and DNA accessibility in EB cells"
output: 
...
io$outdir <- "/Users/ricard/gastrulation/metacc/local_coupling/EB/out"
...
opts$cells <- fread(io$in.sample_metadata) %>% 
  .[pass_accQC==T & pass_metQC==T & culture=="EB",sample] 
```
...
met_ov_dt <- foverlaps(
  ...
  .[,.(met.rate=mean(rate)), by=c("sample","ens_id","window_center","lineage")] %>% droplevels()
```
...
acc_ov_dt <- foverlaps(
  ...
  .[,c("i.rel_start","i.rel_end","rel_start","rel_end"):=NULL] %>%
  .[,.(acc.rate=mean(rate)), by=c("sample","ens_id","window_center","lineage")] %>% droplevels()
```
...
bar <- merge(
  met_ov_dt,
  acc_ov_dt,
  by = c("sample","ens_id","window_center","lineage")
...
```{r}
cor <- bar %>% .[,.(r=cor(x=met.rate, y=acc.rate, method="pearson", use="pairwise.complete.obs")), by=c("sample","window_center","lineage")]
```

<!-- Plot -->
```{r}
p <- ggplot(cor,aes(x=window_center, y=r, color=lineage, fill=lineage)) +
...
pdf(file=paste0(io$outdir,"/metacc_localcoupling_EB.pdf"), width=5, height=3.5)
print(p)
dev.off()
```
