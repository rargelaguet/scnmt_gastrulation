...
sample_metadata <- fread(args$metadata) %>% 
  .[!is.na(celltype) & (pass_metQC==TRUE | pass_accQC==TRUE)]
...
#   merge(sample_metadata[,c("cell","sample","celltype")], by="cell") %>%
...
celltypes.to.subset <- sample_metadata[,.N,by=c("celltype")] %>% .[N>=opts$min_cells] %>% .[,.N,by="celltype"] %>% .[N>1] %>% .$celltype

for (i in celltypes.to.plot) {
  
  # sum(scnmt.peaks %in% marker_peaks.dt[celltype==i & score>=opts$min_marker_score,idx])
  
  tmp <- sample_metadata[celltype%in%celltypes.to.subset,c("cell","celltype")]
  ...
  p.met <- ggplot(to.plot[context=="CG"], aes(x = celltype, y = rate, fill=celltype)) +
  ...
  p.acc <- ggplot(to.plot[context=="GC"], aes(x = celltype, y = rate, fill=celltype)) +
    geom_boxplot(outlier.shape=NA, coef=1) +
    geom_point(position = position_jitterdodge(jitter.width = 0.5), size = 1.25, alpha = 0.75, shape=21) +
    facet_wrap(~celltype) +
...
for (i in celltypes.to.plot) {
  ...
  to.plot <- metacc.dt %>%
    ...
    .[Ntotal>=opts$min_observations] %>%
    .[,rate:=100*(Nmet/Ntotal)] %>%
    merge(sample_metadata[celltype%in%celltypes.to.subset,c("cell","celltype")], by="cell") %>%
    merge(global_rates.dt, by=c("cell","context")) %>%
    .[,rate_norm:=rate/global_rate]
  
  p.met <- ggplot(to.plot[context=="CG"], aes(x = celltype, y = rate_norm, fill=celltype)) +
    geom_boxplot(outlier.shape=NA, coef=1) +
    geom_point(position = position_jitterdodge(jitter.width = 0.5), size = 1.25, alpha = 0.75, shape=21) +
    facet_wrap(~celltype) +
  ...
  p.acc <- ggplot(to.plot[context=="GC"], aes(x = celltype, y = rate_norm, fill=celltype)) +
    geom_boxplot(outlier.shape=NA, coef=1) +
    geom_point(position = position_jitterdodge(jitter.width = 0.5), size = 1.25, alpha = 0.75, shape=21) +
    facet_wrap(~celltype) +
    labs(x="", y="Chr. accessibility levels (%)") +
